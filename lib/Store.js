...
function bind(zenon) {
    ...
    zenon.ev.on('connection.update', update => {
        Object.assign(state, update)
    })

    zenon.ev.on('chats.set', ({ chats: newChats, isLatest }) => {
        /**if (isLatest) {
            chats.clear()
        }**/
    ...
    zenon.ev.on('contacts.set', ({ contacts: newContacts }) => {
    ...
    zenon.ev.on('messages.set', ({ messages: newMessages, isLatest }) => {
        if (isLatest) {
   ...
   zenon.ev.on('contacts.update', updates => {
        for (const update of updates) {
            Object.assign(contacts[update.id], update)
        }
    })

    zenon.ev.on('chats.upsert', newChats => {
        chats.upsert(...newChats)
    })

    zenon.ev.on('messages.delete', item => {
        if ('all' in item) {
    ...
    zenon.ev.on('chats.delete', deletions => {
        for (const item of deletions) {
            chats.deleteById(item)
        }
    })

    zenon.ev.on('chats.update', updates => {
    ...
    zenon.ev.on('presence.update', ({ id, presences: update }) => {
        presences[id] = presences[id] || {}
        Object.assign(presences[id], update)
    })

    zenon.ev.on('messages.upsert', ({ messages: newMessages, type }) => {
        switch (type) {
            case 'append':
            case 'notify':
                for (const msg of newMessages) {
    ...
    /**zenon.ev.on('messages.update', updates => {
    ...
    zenon.ev.on('groups.update', updates => {
        for (const update of updates) {
    ...
    zenon.ev.on('group-participants.update', ({ id, participants, action }) => {
    ...
    zenon.ev.on('message-receipt.update', updates => {
    ...
    return {
        ...
        loadMessages: async (jid, count, cursor) => {
            ...
            const retrieve = async (count, cursor) => {
                const result = await (zenon === null || zenon === void 0 ? void 0 : zenon.fetchMessagesFromWA(jid, count, cursor))
                return result || []
            }
        ...
        loadMessage: async (jid, id) => {
            ...
            if (!message) {
                message = await (zenon === null || zenon === void 0 ? void 0 : zenon.loadMessageFromWA(jid, id))
            }
            return message
        },
        mostRecentMessage: async (jid) => {
            ...
            if (!message) {
                const [result] = await (zenon === null || zenon === void 0 ? void 0 : zenon.fetchMessagesFromWA(jid, 1, undefined))
                message = result
            }
            return message
        },
        fetchImageUrl: async (jid) => {
            ...
            if (!contact) {
                return zenon === null || zenon === void 0 ? void 0 : zenon.profilePictureUrl(jid)
            }
            if (typeof contact.imgUrl === 'undefined') {
                contact.imgUrl = await (zenon === null || zenon === void 0 ? void 0 : zenon.profilePictureUrl(jid))
            }
            return contact.imgUrl
        },
        fetchGroupMetadata: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (zenon === null || zenon === void 0 ? void 0 : zenon.groupMetadata(jid))
            }
            return groupMetadata[jid]
        },
        fetchBroadcastListInfo: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (zenon === null || zenon === void 0 ? void 0 : zenon.getBroadcastListInfo(jid))
            }
            return groupMetadata[jid]
        },
        fetchMessageReceipts: async ({ remoteJid, id }) => {
            ...
            if (!receipts) {
                receipts = await (zenon === null || zenon === void 0 ? void 0 : zenon.messageInfo(remoteJid, id))
                if (msg) {
