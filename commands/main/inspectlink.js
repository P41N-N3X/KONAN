...
module.exports = {
    ...
    start: async(zenon, m, { text }) => {
        ...
        let res = await zenon.groupQueryInvite(code)
...
⬡ *Friends Who Are Known to Join :*\n${res.participants ? res.participants.map((user, i) => ++i + ". @" + user.jid.split("@")[0]).join("\n").trim() : "Not Found"}
        ...
        let pp
        try {
            pp = await zenon.profilePictureUrl(res.id, "image")
        } catch {
            pp = "https://tse2.mm.bing.net/th?id=OIP.n1C1oxOvYLLyDIavrBFoNQHaHa&pid=Api&P=0&w=153&h=153"
        }
        zenon.sendFile(m.from, pp, "", m, { caption: teks, mentions: await zenon.parseMention(teks) })
    },
    isQuery: true
}
