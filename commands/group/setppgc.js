...
module.exports = {
    ...
    noLimit: true,
    start: async(zenon, m, { mime, quoted, text, prefix, command }) => {
		if (!cekAdmin(m.sender)) return global.mess("admin", m)
        if (!cekAdmin(isBotAdmins)) return global.mess("botAdmin", m)
        if (/image/.test(mime)) {
            let media = await zenon.downloadAndSaveMediaMessage(quoted)
            if (text.toLowerCase() === "original") {
                ...
                await zenon.updateProfilePicture(m.from, { url: media })
                .then( res => {
                    m.reply("Sukses")
                    fs.unlinkSync(media)
                }).catch(() => m.reply('err'))
