...
`
            ...
            zenon.sendText(m.from, teks, m)
            if (c && c.toLowerCase() == "--audio") zenon.sendFile(m.from, global.api("zenz", `/islami/quran/audio/${b}`, {}, "apikey"), name.long, m)
...
⭔ Tafsir : ${tafsir.id.long}           
            `
            zenon.sendText(m.from, teks, m)
            if (d && d.toLowerCase() == "--audio") zenon.sendFile(m.from, global.api("zenz", `/islami/quran/audio/${b}/${c}`, {}, "apikey"), surah.name.long, m)
        } else {
            return m.reply(`
Example :
