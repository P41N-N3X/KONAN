...
module.exports = {
	...
	noLimit: true,
    start: async(zenon, m, {isPremium, isOwner}) => {
		let statuses
		try {
			statuses = await zenon.fetchStatus(m.sender);
		} catch {
			statuses = "Nothing.."
		}
		...
		try {
			var pp = await zenon.profilePictureUrl(m.sender, "image");
		} catch {
		...
		caption += `│\n`
		caption += `└───────⭓\n`
		zenon.sendFile(m.from, pp, "", m, { caption })
    }
} 