...
const time = require("moment-timezone").tz('Asia/Kolkata').format('DD/MM HH:mm:ss')
...
const expiredCheck = (zenon, m, _db) => {
	setInterval(() => {
		let found = false
		Object.keys(_db).forEach((i) => {
		...
		if (found !== false) {
			...
			fs.writeFileSync("./database/user.json", JSON.stringify(_db, null, 4))
            idny ? zenon.sendText(idny, "Your Premium Role has run out", m) : ""
			idny = false
		}
	}, 1000)
}
