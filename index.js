...
const connect = async () => {
    ...
    const zenon = new WAConnection(global.opts["legacy"] ? makeWALegacySocket(connOptions) : makeWASocket(connOptions))
    global.Store = Store.bind(zenon)

    zenon.ev.on("creds.update", saveState)

    zenon.ev.on("connection.update", async(update) => {
        if (update.connection == "open" && zenon.type == "legacy") {
            zenon.user = {
                id: zenon.state.legacy.user.id,
                jid: zenon.state.legacy.user.id,
                name: zenon.state.legacy.user.name
            }
        }
        ...
        if (connection == "close") {
            ...
            if (reason === DisconnectReason.badSession) { console.log(`Bad Session File, Please Delete Session and Scan Again`); zenon.logout(); }
            ...
            else if (reason === DisconnectReason.connectionReplaced) { console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); zenon.logout(); }
            ...
            else zenon.end(`Unknown DisconnectReason: ${reason}|${connection}`)
        }
    })

    zenon.ev.on("messages.upsert", async (chatUpdate) => {
        m = serialize(zenon, chatUpdate.messages[0])

        if (!m.message) return
        if (m.key && m.key.remoteJid == "status@broadcast") return
        if (m.key.id.startsWith("BAE5") && m.key.id.length == 16) return
        require("./zenon")(zenon, m, Commands, chatUpdate)
    })

    if (zenon.user && zenon.user?.id) zenon.user.jid = jidNormalizedUser(zenon.user?.id)
    zenon.logger = (zenon.type == "legacy") ? DEFAULT_LEGACY_CONNECTION_CONFIG.logger.child({ }) : DEFAULT_CONNECTION_CONFIG.logger.child({ })
}
